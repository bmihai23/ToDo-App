...
const TodoForm = ({addTodo}) =>{
    ...
    const handleChange = ( event ) => {   //to handle change in form input (when user is typing)
        setvalue(event.target.value);         //e.target gives the value of current value the user is typing
    };

    const handleSubmit = ( event ) => {
        event.preventDefault();     //to prevent DOM from reloading the page
